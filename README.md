## Hey 👋, I'm Yamen Brieghish

<a href='https://www.linkedin.com/in/yamen-m-brieghish/'><img align='left' alt="linkedin" src="https://raw.githubusercontent.com/rahul-jha98/rahul-jha98/561d474902b59c7429ec22bb73e225696c27b202/assets/linkedin.svg" height='18px'/></a>

<br/>
<br/>

I am a Software Engineering graduate who loves building practical applications and improving user experience through clean design and efficient code. I enjoy working on both mobile and web-based projects and continuously expanding my technical skills.

<br/>

### 🧐 More About Me:

- 🎓 &nbsp; Bachelor in Software Engineering – Philadelphia University (2021–2025)
- 📍 &nbsp; Based in Irbid, Jordan
- 💻 &nbsp; Working with: **HTML, CSS, C#, C++, Java**
- 🔧 &nbsp; Tools & Tech: **.NET, Firebase, SQL Server, Figma**
- 🌱 &nbsp; Currently improving my skills in web development and clean code practices
- 👨🏻‍💻 &nbsp; All of my projects are available on my https://github.com/YamenBrieghish?tab=repositories
- 📫 &nbsp; How to reach me: **yamenbrieghish@gmail.com**

---

### 🛠️ Projects

#### 🏋️ GVR – Graduate Project
Mobile application that connects users with gym, volunteer and recreational activities.  
Features include search, favorites, maps, authentication and personalized user data.  
**Tech Used:** Java (Android), Firebase, XML  

#### 🎓 EduLive – UI/UX Design
Interactive e-learning platform prototype to improve accessibility and user experience.  
Designed instructor profiles and achievement pages using reusable components.  
**Tech Used:** Figma, Responsive Design  

#### 🏫 Student Management System
System for managing students, courses and lecturers with full database integration.  
Built complete CRUD operations and ER diagram.  
**Tech Used:** C#, SQL Server  

---

### 💼 Experience

**Shopify Theme Customization (2025 – Present)**  
- Customizing product pages and homepage layouts  
- Improving responsiveness and performance  
- Enhancing user experience and conversions through clean HTML, CSS & JavaScript  

---

### 🔨 Languages and Tools

<p>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/cplusplus/cplusplus-original.svg" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/csharp/csharp-original.svg" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/firebase/firebase-plain.svg" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/dot-net/dot-net-original.svg" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/figma/figma-original.svg" height="40"/>
</p>


